...
struct fssynccb_struct {
    void (* create)(struct workspace_object_struct *object, struct entry_s *entry, uint32_t fssync_mask);
    void (* change)(struct workspace_object_struct *object, struct entry_s *entry, uint32_t event_mask, uint32_t fssync_mask);
    void (* remove)(struct workspace_object_struct *object, struct entry_s *entry, uint32_t fssync_mask);
};
...
static void synchronize_directory_full(struct workspace_object_struct *object, struct entry_s *parent, struct directory_s *directory, unsigned int fd, struct readdir_struct *readdir, struct timespec *synctime, unsigned int len, struct fssynccb_struct *fssynccb, uint32_t fssync_mask, unsigned int *error)
{
	...
	*error=0;
	...
	if (entry && inode) {
		...
		adjust_pathmax(object->workspace_mount, len + 1 + xname.len);
...
static void synchronize_directory_simple(struct workspace_object_struct *object, struct entry_s *parent, struct directory_s *directory, unsigned int fd, struct readdir_struct *readdir, struct timespec *synctime, unsigned int len, struct fssynccb_struct *fssynccb, uint32_t fssync_mask, unsigned int *error)
{
    struct name_s xname={NULL, 0, 0};
    unsigned char dtype=0;
    int res;
...
static void synchronize_directory_virtual(struct workspace_object_struct *object, struct entry_s *parent, struct directory_s *directory, unsigned int fd, struct timespec *synctime, struct fssynccb_struct *fssynccb, uint32_t fssync_mask, unsigned int *error)
{
...
void remove_entries_notfound(struct workspace_object_struct *object, struct directory_s *directory, struct timespec *synctime, struct fssynccb_struct *fssynccb, uint32_t sync_mask)
{
    struct entry_s *entry=NULL;
...
/*
    generic function to synchronize a directory
    it checks what synchronize function to call (which will do the work)

    parameter:

    - pinode: the inode of the dorectory to synchronize
    - fssynccb: function which is called when the cache differs from the underlying fs (with mask fsnotify_mask)
    - fsnotify_mask: the mask to compare
    - path: path of the underlying directory
    - len: maximum length of path so far
    - object: the cache is related to this object

    this function will call different synchronize workers when:

    - on linux when the underlying fs is virtual like sysfs and procfs use readdir
    - on all other cases use getdents

    - when directory has not been synced before do a full sync
    - when underlying directory has changed (on linux) entries are added and/or removed, do a simple sync
    - on all other cases do a virtual sync
*/

static void synchronize_directory(struct inode_s *pinode, struct fssynccb_struct *fssynccb, uint32_t fsnotify_mask, char *path, unsigned int len, struct workspace_object_struct *object)
{
...
void fssync_remove_entry_cb_ignore(struct workspace_object_struct *object, struct entry_s *entry, uint32_t mask)
{
}
...
static void process_fssync_job(void *data)
{
	...
	struct inode_s *inode=watch->inode;
...
static void run_fssync(void *data)
{
	...
	*/
	...
	work_workerthread(NULL, -1, process_fssync_job, NULL, &error);
